 ...
 */
 export async function authClientFactory(): Promise<ConfidentialClientApplication> {
    const tenantId = process.env.AAD_MSAL_AUTH_TENANT_ID;
    const clientId = process.env.AAD_MSAL_AUTH_ID;
    const clientSecret = process.env.AAD_MSAL_AUTH_SECRET;
 ...
 */
 export async function initHandler(req: IncomingMessage & { session: IronSession }, res: ServerResponse): Promise<[string, IActivationProps, string | null] | never> {
    // get our request query
    const query = (new URL(req.url, process.env.NEXT_PUBLIC_FILEHANDLER_SITE_HOST_URL)).searchParams;
    ...
    if (token) {
        ...
        req.session[sessionKey] = sessionData;
        await req.session.save();

        return [token, loginState.activationParams, null];
    }

    const session: SessionState = req.session[sessionKey];
